...
public class VBORenderer
{
	...
	{
		if (JumpAndJan.DISPLAY_WIDTH != oldWidth
				|| JumpAndJan.DISPLAY_HEIGHT != oldHeight)
			projection();
	}
	...
	private static void projection()
	{
		...
		if (dimension == 3)
		{
			gluPerspective(30, (float) JumpAndJan.DISPLAY_WIDTH
					/ (float) JumpAndJan.DISPLAY_HEIGHT, 0.01f, 2500f);
			// glScalef(-1, -1, 1);
		} else if (dimension == 2)
		{
			glOrtho(0, JumpAndJan.DISPLAY_WIDTH, JumpAndJan.DISPLAY_HEIGHT, 0, 1,
					-1);

		}
		glMatrixMode(GL_MODELVIEW);
		glLoadIdentity();
		oldWidth = JumpAndJan.DISPLAY_WIDTH;
		oldWidth = JumpAndJan.DISPLAY_HEIGHT;
	}
