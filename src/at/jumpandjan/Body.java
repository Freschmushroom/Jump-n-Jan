...
public class Body
{
	 ...
	 */
	public Body(double x, double y, double width, double height, Level level)
	{
	...
	{
		return bounds.intersects(level.camera);
	}
	...
	public void render()
	{
		render("/none", bounds.width, bounds.height, bounds.x, bounds.y, (float) bounds.width, (float) bounds.height, false);
	}
	 ...
	 *            Texture
	 * @param width
	 *            Width of the drawn area as well as the texture's
	 * @param height
	 *            Height of the drawn area as well as the texture's
	 * @param x
	 *            The x-coordinate in OpenGL
	 * @param y
	 *            The y-coordinate in OpenGL
	 * @param rWidth
	...
	{
		render(texture, width, height, x, y, 0, 0, rWidth, rHeight, sWidth, sHeight, turned);
	}
	 ...
	 *            Flipped
	 */
	public void render(String texture, double width, double height, double x, double y, float rX, float rY, float rWidth, float rHeight, float sWidth, float sHeight, boolean turned)
	{
		glPushMatrix();
		glTranslated(x, y, 0);
		if (turned)
		...
		TextureManager.instance.bindTexture(TextureManager.instance.getTexture(texture));
		float f = 1 / sWidth;
		float f1 = 1 / sHeight;
		glBegin(GL_QUADS);
	...
	/**
	 * Convenience method. Casts all arguments to integers, then passes them on
	 * to {@link setBounds(int, int, int, int)}
	 */
	 ...
	 * 
	 * @param bounds
	 *            The bounds which should be overtaken
	 * @throws NullPointerException
	 *             Throws, if {@code bounds == null}
	 */
	...
	static
	{
		Out.inf(Body.class, "01.06.2013", "Michael, Felix", null);
	}
}
