...
public class CompButton extends Component
{
	...
	/**
	 * If the button is disabled, no input will be accepted and it will be
	...
	/**
	 * Centers the button at the specified location. Convenience method for
	 * layouting.
	 * 
	 * @param x
	...
	/**
	 * Enables this button. Equivalent to setEnabled(true). Convenience method
	 */
	public void enable()
	{
		setEnabled(true);
	...
	/**
	 * Disables this button. Equivalent to setEnabled(false). Convenience method
	 */
	public void disable()
	{
		setEnabled(false);
	...
	/**
	 * Sets the width of this component. The minimum width is 20; if width is
	 * smaller than 20, it is clamped.
	 */
	@Override
	...
	public void drawComponent()
	{
		...
		addVertexUV(getX() + 16, getY(), 16, 0 + yOffset);
		addVertexUV(getX() + 16, getY() + getHeight(), 16, 32 + yOffset);
		addVertexUV(getX() + getWidth() - 16, getY() + getHeight(), 256 - 16, 32 + yOffset);
		addVertexUV(getX() + getWidth() - 16, getY(), 256 - 16, 0 + yOffset);

		addVertexUV(getX() + getWidth() - 16, getY(), 256 - 16, 0 + yOffset);
		addVertexUV(getX() + getWidth() - 16, getY() + getHeight(), 256 - 16, 32 + yOffset);
		addVertexUV(getX() + getWidth(), getY() + getHeight(), 256, 32 + yOffset);
		addVertexUV(getX() + getWidth(), getY(), 256, 0 + yOffset);

		end();
		
		font.drawString(this.getX() + this.getWidth() / 2, this.getY() + this.getHeight() * 0.85f, text, 1, -1, TrueTypeFont.ALIGN_CENTER);
