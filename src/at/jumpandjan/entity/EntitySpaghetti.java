...
/**
 * The first boss opponent in Jump'n'Jan. It runs away from the player and
 * spawns Meatballs on its way.
 * 
 * @author Felix
 *
 */
public class EntitySpaghetti extends Entity implements FlagBoss
{
	 ...
	 * @param y
	 *            the y coordiante of the top left corner
	 * @param level
	 *            the level the entity spawns in
	 */
	...
	{
		super.update();
		if (Math.abs(level.getPlayer().getPivotX() - this.getPivotX()) < 200 && Math.abs(level.getPlayer().getPivotX() - this.getPivotX()) > 1)
		{
			motion.x = (float) (-(level.getPlayer().getPivotX() - this.getPivotX()) / Math.abs(level.getPlayer().getPivotX() - this.getPivotX()) * 2.75f);
