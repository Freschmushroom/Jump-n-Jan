	...
	public void update()
	{
		...
		if (motion.length() != 0)
		{
			if (motion.x != 0)
			{
				...
				while (!arrived)
				{
					for (Body o : level.collisionPool)
					{
						if (!this.checkCollisionWith(o))
						{
							continue;
						}
					...
					if (!arrived)
					{
						arrived = Math.abs(tempBounds.x - startX) == Math.abs((int) motion.x);
						bounds = (Rectangle) tempBounds.clone();
						tempBounds.setLocation(tempBounds.x + direction, tempBounds.y);
					}
				}
			}
			if (motion.y != 0)
			{
				...
				while (!arrived)
				{
					...
					if (!arrived)
					{
						arrived = Math.abs(tempBounds.y - startY) == Math.abs((int) motion.y);
						bounds = (Rectangle) tempBounds.clone();
						tempBounds.setLocation(tempBounds.x, tempBounds.y + direction);
					}
				}
			}
		}
